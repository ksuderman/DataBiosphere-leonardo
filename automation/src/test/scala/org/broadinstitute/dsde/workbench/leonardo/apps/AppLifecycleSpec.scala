...
//  }
//
//  "create and delete a Cromwell app" taggedAs Retryable in { _ =>
//    withNewProject { googleProject =>
//      val appName = randomAppName
//
//      val createAppRequest = defaultCreateAppRequest.copy(
...
//      LeonardoApiClient.client.use { implicit client =>
//        for {
//          _ <- loggerIO.info(s"CustomAppCreationSpec: About to create Cromwell app ${googleProject.value}/${appName.value}")
//
//          // Create the app
//          _ <- LeonardoApiClient.createApp(googleProject, appName, createAppRequest)
//
...
//            120,
//            10 seconds,
//            s"CustomAppCreationSpec: Cromwell app ${googleProject.value}/${appName.value} did not finish creating after 20 minutes"
//          )(implicitly, appInStateOrError(AppStatus.Running))
//          _ <- loggerIO.info(
//            s"CustomAppCreationSpec: Cromwell app ${googleProject.value}/${appName.value} monitor result: ${monitorCreateResult}"
//          )
//          _ = monitorCreateResult.status shouldBe AppStatus.Running
//
//          _ <- IO.sleep(1 minute)
...
//
//          _ <- loggerIO.info(
//            s"CustomAppCreationSpec: Cromwell app ${googleProject.value}/${appName.value} delete result: $monitorDeleteResult"
//          )
//
//          _ = monitorDeleteResult.map(_.status) shouldBe List(AppStatus.Deleted)
//        } yield ()
